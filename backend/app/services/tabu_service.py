...
class TabuSearchOptimizer:
    ...
    @staticmethod
    def _parse_time(time_val: Any) -> Optional[float]:
        """Chuyển đổi time window từ chuỗi HH:MM hoặc số thành float phút"""
        if time_val is None:
            return None
    ...
    def calculate_distance(p1: Dict[str, Any], p2: Dict[str, Any]) -> float:
        ...
        lat1 = p1.get("latitude", p1.get("lat", 0.0))
        lng1 = p1.get("longitude", p1.get("lng", 0.0))
        lat2 = p2.get("latitude", p2.get("lat", 0.0))
        lng2 = p2.get("longitude", p2.get("lng", 0.0))
        # Đơn vị xấp xỉ km
    ...
    def evaluate_route(
        ...
        point_index_map: Optional[Dict[str, int]] = None,
        departure_time_mins: float = 0.0
    ) -> RouteEvaluation:
        """
        Hàm lượng giá (Cost/Fitness function) cho VRPTW theo ma trận thật (nếu có) hoặc công thức tọa độ:
        - Sử dụng distance_matrix & travel_time_matrix thật từ OSRM/Goong nếu được cung cấp.
        ...
        current_time = float(departure_time_mins)
        current_point = depot

        depot_id = str(depot.get("id", "depot"))

        for stop in route:
            stop_id = str(stop.get("id", ""))
            # 1. Tính khoảng cách và thời gian di chuyển theo ma trận thật nếu có
            if (
                distance_matrix is not None and travel_time_matrix is not None and
                point_index_map is not None and
                str(current_point.get("id", "")) in point_index_map and
                stop_id in point_index_map
            ):
                u = point_index_map[str(current_point.get("id", ""))]
                v = point_index_map[stop_id]
                dist = distance_matrix[u][v]
                travel_time = travel_time_matrix[u][v]
            else:
            ...
            tw_start = self._parse_time(stop.get("time_window_start"))
            tw_end = self._parse_time(stop.get("time_window_end"))
        ...
        if (
            distance_matrix is not None and point_index_map is not None and
            str(current_point.get("id", "")) in point_index_map and
            depot_id in point_index_map
        ):
            u = point_index_map[str(current_point.get("id", ""))]
            v = point_index_map[depot_id]
            return_dist = distance_matrix[u][v]
        else:
    ...
    def optimize_cluster_route(
        self,
        cluster_stops: List[Dict[str, Any]],
        depot: Dict[str, Any],
        distance_matrix: Optional[List[List[float]]] = None,
        travel_time_matrix: Optional[List[List[float]]] = None,
        point_index_map: Optional[Dict[str, int]] = None,
        ...
        """
        Tối ưu thứ tự điểm dừng cho 1 cụm xe bằng thuật toán Tabu Search theo ma trận thật:
        - Tích hợp ma trận khoảng cách & thời gian thật (OSRM/Goong/Static).
        ...
        if len(cluster_stops) <= 2:
            eval_res = self.evaluate_route(
                cluster_stops, depot, distance_matrix, travel_time_matrix, point_index_map, departure_time_mins
            )
        ...
        best_eval = self.evaluate_route(
            best_solution, depot, distance_matrix, travel_time_matrix, point_index_map, departure_time_mins
        )
        best_cost = best_eval.cost

        current_solution = list(best_solution)

        # 1. Tính Tabu Tenure Động
        ...
        for iteration in range(self.max_iterations):
            ...
            # 2. Sinh không gian lân cận (Neighborhood moves: Swap & 2-Opt)
            candidate_moves = []
            for i in range(n - 1):
            ...
            # 3. Đánh giá tất cả các nước đi trong không gian lân cận theo ma trận thật
            for move in candidate_moves:
                neighbor_solution = move.apply(current_solution)
                neighbor_eval = self.evaluate_route(
                    neighbor_solution, depot, distance_matrix, travel_time_matrix, point_index_map, departure_time_mins
                )
            ...
            if iterations_without_improvement == self.diversify_rounds:
                ...
                random_eval = self.evaluate_route(
                    current_solution, depot, distance_matrix, travel_time_matrix, point_index_map, departure_time_mins
                )

...
Strategy:
- Use uuid.UUID objects for all UUID columns to ensure SQLAlchemy & SQLite compatibility.
- Test route_worker and RBAC logic directly.
- Solver uses static matrix to bypass OSRM in CI/test.
"""
import uuid
import datetime
import pytest
...
def test_ticket_reserve_and_cancel(db_session):
    """
...
def test_route_worker_creates_route_stops(db_session):
    """
    ...
    result_job = run_route_job_worker(db=db, job_id=job_id)
    ...
    db.refresh(ticket)
    assert ticket.status == TicketStatus.ASSIGNED, "Ticket phải chuyển sang ASSIGNED"
    assert ticket.route_id is not None, "Ticket phải có route_id"

    # BUG-VRPTW-01 regression: đếm route_stops
    stop_count = db.query(RouteStop).filter(RouteStop.route_id == ticket.route_id).count()
...
def test_rbac_student_sees_only_own_route(db_session):
    """
...
# ── Test 5: Driver RBAC ───────────────────────────────────────────────────────
def test_rbac_driver_sees_own_vehicle_route(db_session):
    """
    ...
    assert driver_a_routes[0].id == route_a_id
    ...
    assert driver_b_routes[0].id == route_b_id


# ── Test 6: Job idempotency — chạy 2 lần không tạo route trùng ───────────────
def test_route_job_no_duplicate_on_rerun(db_session):
    """
    ...
    # Lần 1
    result1 = run_route_job_worker(db=db, job_id=job_id)
    assert result1.status == RouteJobStatus.SUCCEEDED
    ...
    result2 = run_route_job_worker(db=db, job_id=job_id)
    assert result2.status == RouteJobStatus.SUCCEEDED
